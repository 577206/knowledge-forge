...
- **Final Exam Review** prepares Agent-readable source packs and prompts; the formal review is generated by a local Agent that reads the chunks.
...
- Study guide fallback draft
- Quiz fallback draft
- Flashcards fallback draft
- Markdown note
- Keywords
- Candidate tags
- Concept candidates
...
Local output is useful for preview/offline fallback, but it must be marked as draft. Do not present local-rules output as deep AI understanding or as the formal final report.
...
Google NotebookLM integration for optional deep source-grounded reading. It complements, but does not replace, the Agent-first formal output path.
...
This is the formal generation layer. Knowledge Forge assumes the user has an AI agent for high-quality study packs/reports; local rules are fallback only.
...
## Generation honesty matrix

| Path | Status | Use for |
|---|---|---|
| Agent via Claude Code / Codex | Formal path | Full review packs, reports, PDF source Markdown |
| Local `local-rules` | Fallback draft | Fast preview, no-Agent environments, smoke tests |
| NotebookLM | Optional enhancement | Source-grounded reading/Q&A, manual outputs to capture and rewrite |
| Codex | Formal optional UI runner | Generate study packs/custom tasks from agent-pack chunks |


---

## Agent / Tools dependency guide

For a clean computer with no Agent configured, read `docs/AGENT_TOOLS_AND_SKILLS.md`. It lists required, recommended, optional, and forbidden tools/skills, plus verification commands.
